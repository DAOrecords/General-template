...
export async function getBuyableTokens() {
  ...
  await window.wallet.viewMethod({ method: 'nft_tokens', contractId: contract, args: options })
    .then((response) => {    
      ...
      inVault = response.filter((nft) => nft.owner_id === contract);
    ...
    const nextNFTs = await Promise.all(rootIDs.map(async (id) => { // We get the next buyable NFT for each root (lowest generation)
      const result = await getNextBuyableInstance(contract, id);
      return result;
    }))
...
export async function getStakedNFTsForUser(){
  var result = [];
  var contract = STAKING_CONTRACT;
  const options = {
    owner: window.accountId
  }
  await window.wallet.viewMethod({ method: 'get_all_stakedNFTs_for_owner', args: options, contractId: contract })
    .then((response) => {
      if(response.length > 0){
        result = response.map((item) => {
          return {
            ...item,
            contract: item.issuing_contract,
...
export async function getStakedNFTRewards(staked_nft_id){  
  var result = -2;
  var contract = STAKING_CONTRACT;
  const options = {
    staked_nft_id
  }
  await window.wallet.viewMethod({ method: 'get_stakedNFT_rewards', args: options, contractId: contract })
    .then((response) => {
      result = response.rewards_accumulated / 100000000;
    })
    .catch((err) => console.error(err));
